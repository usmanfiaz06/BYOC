    ...
    <footer className="bg-coffee-dark">
      <div className="max-w-[1400px] mx-auto px-6 lg:px-10 py-20">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-14">
          ...
          <div>
            ...
            <div className="flex gap-3">
              {[
                { label: 'LinkedIn', href: 'https://www.linkedin.com/company/byoc-global', icon: 'in' },
                { label: 'Instagram', href: 'https://instagram.com/byoc.global', icon: 'ig' },
                { label: 'X', href: 'https://twitter.com/byocglobal', icon: 'x' },
              ].map((s) => (
                <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded-full border border-cream/10 flex items-center justify-center text-[10px] text-cream/40 hover:border-accent hover:text-accent transition-colors tracking-wider uppercase" aria-label={s.label}>
                  {s.icon}
                </a>
              ))}
              <a href="mailto:hello@byoc.global" className="w-8 h-8 rounded-full border border-cream/10 flex items-center justify-center text-cream/40 hover:border-accent hover:text-accent transition-colors" aria-label="Email">
                <Mail size={12} />
              </a>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {[
                ...
                <Link key={link.href} href={link.href} className="block text-[12px] text-cream/35 hover:text-cream/70 transition-colors">
                  {link.label}
                </Link>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="text-[11px] text-cream/15 tracking-[0.03em]">
              hello@byoc.global
            </div>
          </div>
        </div>
        ...
        <div className="mt-20 pt-6 border-t border-cream/8 flex flex-col md:flex-row items-center justify-between gap-3">
          ...
          <p className="text-[11px] text-cream/15">21+ countries · 80+ gatherings · 3,200+ members</p>
        </div>
      </div>
    </footer>
  );
